...
import { getAuth, GoogleAuthProvider, signInWithCredential, onAuthStateChanged, signOut as fbSignOut } from "https://www.gstatic.com/firebasejs/11.6.0/firebase-auth.js";
...
const GOOGLE_CLIENT_ID = "437530037218-8irqh4be0iel138frjj2hpru7sfloi50.apps.googleusercontent.com";
...
// ── Auth：Google One Tap（最相容 Safari/iOS）──
window.signInWithGoogle = () => {
  // One Tap 回調
  window.handleGoogleCredential = (response) => {
    const credential = GoogleAuthProvider.credential(response.credential);
    signInWithCredential(auth, credential)
      .catch(e => showToast('登入失敗：' + e.code));
  };
...
document.addEventListener('DOMContentLoaded', () => {
  // 等 Google SDK 載入後渲染官方按鈕
  const tryRenderBtn = (retry = 0) => {
    if (window.google && window.google.accounts) {
      google.accounts.id.initialize({
        client_id: GOOGLE_CLIENT_ID,
        callback: window.handleGoogleCredential,
      ...
      // 渲染官方 Google 按鈕（最相容 iOS Safari）
      google.accounts.id.renderButton(
        document.getElementById('google-btn-container'),
        { type:'standard', shape:'pill', theme:'outline',
          text:'signin_with', size:'large', locale:'zh_TW', width:280 }
      );
      // 隱藏備用按鈕
      const fallback = document.getElementById('google-login-btn');
      if (fallback) fallback.style.display = 'none';
    } else if (retry < 20) {
      // SDK 還沒載入，等 500ms 再試
      setTimeout(() => tryRenderBtn(retry + 1), 500);
    }
  };
  tryRenderBtn();
